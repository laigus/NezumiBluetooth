...
频率文件是一个体积较小、可校验、可通过微信聊天分享的 JSON 文件。手机导入或手动模式录制后直接写入微信本地存储；增加本机频率不涉及小程序上传与体验版发布。
...
单个频率最长 60 分钟，单个 JSON 最多 2 MB，本机最多保存 100 个本机频率。
...
5. 导入成功后，新条目出现在频率列表中，并带“本机”标签。

同一日程哈希只保留一份。ID 与已有条目同名但内容不同，导入器会自动追加 `-2`、`-3` 等后缀。

也可以在 COCO 的“手动模式”点击“创建频率”，直接记录参数变化并保存。录制结果使用同一结构、校验和本机去重规则。

本机频率可通过详情页底部的操作按钮改名或删除，列表页只保留播放操作。改名不改变 `id`、帧日程或摘要；删除只移除微信本地存储中的对应条目。随源码发布的内置频率保持只读。
